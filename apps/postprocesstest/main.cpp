...
int main(int argc,char** argv) {
    ...
    for(int i=1;i<argc;++i){strict|=std::string(argv[i])=="--require-gpu";require_gl|=std::string(argv[i])=="--require-opengl";}
    strict|=require_gl;
    ...
    if(!window){std::fprintf(stderr,"SKIP: %s\n",SDL_GetError());return strict?1:77;}
    {
        ...
        // Precomposed fields must bypass the explicit second merge too.
        ...
        require(dark[(32*512+32)*4]<3,"already-composed guest fields are never averaged twice");
