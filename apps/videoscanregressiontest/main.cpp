...
void discarded_samples(VideoScanGeneratorIntf& decoder, ScanBuffer& scans,
    ...
    // Appletini's SHR/A2Li path can discard a frame containing VSync. Returning
    // to the cycle decoder must retain its image until a new sync is observed.
    scans.clear();
    samples(scans, 40, 0x00);
    decoder.generate_frame(&scans);
    expect(row_is(output, line, start, 255),
           "discarded extension samples do not append to stale decoder state");
    marker(scans, VM_VSYNC);
    samples(scans, 40, 0x00);
    decoder.generate_frame(&scans);
    decoder.generate_frame(&scans);
