...
vec4 HalveFrameRate(vec2 coords, vec4 currentColor)
{
    ...
    if (_683)
    {
        ...
        vec3 previousColor = s2l(param_1);
        vec3 linearMix = (currentColor.xyz + previousColor) * 0.5;
...
vec3 l2oklab(vec3 rgb)
{
    ...
    vec3 lms = mat3(vec3(0.4122214615345001220703125, 0.21190349757671356201171875, 0.0883024632930755615234375), vec3(0.536332547664642333984375, 0.680699527263641357421875, 0.28171885013580322265625), vec3(0.0514459945261478424072265625, 0.10739696025848388671875, 0.629978716373443603515625)) * rgb;
    vec3 param = lms;
...
vec3 oklab2l(vec3 lab)
{
    ...
    vec3 lms = mat3(vec3(1.0), vec3(0.3963377773761749267578125, -0.1055613458156585693359375, -0.089484177529811859130859375), vec3(0.21580375730991363525390625, -0.06385417282581329345703125, -1.2914855480194091796875)) * lab;
...
vec4 GenerateGhosting(vec2 coords, inout vec4 currentColor)
{
    ...
    {
        float currentIntensity = dot(currentColor.xyz, vec3(0.2125999927520751953125, 0.715200006961822509765625, 0.072200000286102294921875));
        float previousIntensity = dot(previousColor.xyz, vec3(0.2125999927520751953125, 0.715200006961822509765625, 0.072200000286102294921875));
