#version 450
// Port of SuperDuperDisplay 0.9.1 b49a491. GPL-2.0-or-later; original authors in reference.
layout(set=3,binding=0,std140) uniform Context { vec4 u[18]; };
layout(set=2,binding=0) uniform sampler2D A2TextureCurrent;
layout(set=2,binding=1) uniform sampler2D PreviousFrame;
layout(location=0) in vec2 vUV;
layout(location=0) out vec4 FragColor;
#define pi 3.1415926535897932384626433
#define InputSize (u[0].xy)
#define TextureSize (u[0].xy)
#define OutputSize (u[0].zw)
#define ScanlineCount (uint(u[1].x))
#define iFrameCount (int(u[1].y))
#define iFrameMergeCount (int(u[1].z))
#define POSTPROCESSING_LEVEL (int(u[1].w))
#define GhostingPercent (u[2].x)
#define BlurSize (u[2].y)
#define bBlurGlow ((u[2].z > 0.5))
#define bUseOKlab ((u[2].w > 0.5))
#define bCORNER_SMOOTH ((u[3].x > 0.5))
#define bSLOT ((u[3].y > 0.5))
#define BARRELDISTORTION (u[3].z)
#define BGR (u[3].w)
#define BLACK (u[4].x)
#define BR_DEP (u[4].y)
#define BRIGHTNESS (u[4].z)
#define CONTRAST (u[4].w)
#define CONV_B (u[5].x)
#define CONV_G (u[5].y)
#define CONV_R (u[5].z)
#define CORNER (u[5].w)
...
#define MASKL (u[7].z)
#define MSIZE (u[7].w)
#define SATURATION (u[8].x)
#define SCANLINE_WEIGHT (u[8].y)
#define SCAN_SPEED (u[8].z)
#define FILM_GRAIN (u[8].w)
#define INTERLACE_WEIGHT (u[9].x)
#define SLOTW (u[9].y)
#define VIGNETTE_WEIGHT (u[9].z)
#define iCOLOR_SPACE (int(u[9].w))
#define iM_TYPE (int(u[10].x))
#define iSCANLINE_TYPE (int(u[10].y))
#define vWARP (u[10].zw)
#define bHalveFrameRate ((u[16].y > 0.5))
#define bPOTATO (false)
#define bVIGNETTE (false)
...
{
	if (u[16].x > 0.5 && (iFrameMergeCount & 1) == 1) {
		vec3 previousColor = s2l(SamplePrevious( coords).rgb);
...
{
	if (u[16].x < 0.5) return currentColor;
	float ghosting = GhostingPercent/100.0;
	vec4 blended = vec4(0.0,0.0,0.0,0.0);
	vec4 previousColor = SamplePrevious( coords);
...
vec4 PhosphorBlur(sampler2D tex, vec2 uv, vec2 resolution, float blurAmount) {
	vec4 color = SampleCurrentLod(uv, blurAmount * 4.0);
	color.rgb = s2l(color.rgb);

	// To get a glowing style, we overlay the regular texture data at 30%
	if (bBlurGlow)
		color.rgb = mix(color.rgb, s2l(SampleCurrent(uv).rgb), 0.3);
...
void main() {
    TexCoords = (vUV-u[12].xy)/u[12].zw;
    FragColor = vec4(0.0,0.0,0.0,1.0);
    if (any(lessThan(TexCoords,vec2(0.0))) || any(greaterThan(TexCoords,vec2(1.0)))) return;


    if (POSTPROCESSING_LEVEL <= 1) {
        vec4 color = SampleCurrent(TexCoords);
        if (POSTPROCESSING_LEVEL == 1) color.rgb *= 1.0-mod(floor(TexCoords.y*TextureSize.y),2.0);
        color.rgb = s2l(color.rgb);
        if (bHalveFrameRate) color = HalveFrameRate(TexCoords,color);
        FragColor = vec4(l2s(color.rgb),1.0);
        return;
    }
	...
	if (POSTPROCESSING_LEVEL == 1) {
		FragColor = SampleCurrent( TexCoords);
	...
	if (BlurSize > 0.001)
		...
		res0 = SampleCurrent(pos);
		res0.rgb = s2l(res0.rgb);
	}

	res = res0.rgb;
    if (res0.a <= 0.0) return;
	// Convergence
	if (C_STR > 0.0001) {
		if ((abs(CONV_R)+abs(CONV_G)+abs(CONV_B)) > 0.001) {
			float resr = SampleCurrent(pos + dx*CONV_R).r;
			float resg = SampleCurrent(pos + dx*CONV_G).g;
			float resb = SampleCurrent(pos + dx*CONV_B).b;
