...
| `"voc"` | `DEVICE_ID_VOC` | Video Overlay Card | IIgs only, slot 3 only; see [Cards_VOC.md](Cards_VOC.md) |
...
| `"serial"` | Real host UART; `path` is the OS port name (macOS `/dev/cu.*`, Windows `COMn`, Linux `/dev/ttyUSB*`) |
