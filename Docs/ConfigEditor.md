...
* **Slots** — click a slot to pick which card goes there (or None). Card choices include Disk II, BazFast, Appletini (slot 7), Mockingboard, [Uthernet II](Cards_UthernetII.md), [Super Serial](Cards_SuperSerial.md), [Apple Mouse](Cards_AppleMouse.md), [Parallel](Cards_Parallel.md), and others.
* **Storage (pre-mount)** — click a drive to choose a disk image that will be mounted when you later launch this config. Click again to clear it. Any valid disk image including .pmap (BazFast/Appletini multi-image) can be "pre-mounted" this way.
* **Serial / Parallel** — click a port button and choose **None**, **File**, or **Modem**. See [Serial & Parallel Connections](SerialConnections.md).
